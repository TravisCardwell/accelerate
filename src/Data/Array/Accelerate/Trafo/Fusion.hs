...
--   where
--     go (Step (ArrayR shR _) sh ix g v) = Step  (ArrayR shR tR) sh ix (sinkArrayInstr env f `compose` g) v
--     go (Yield (ArrayR shR _) sh g)     = Yield (ArrayR shR tR) sh    (sinkArrayInstr env f `compose` g)
